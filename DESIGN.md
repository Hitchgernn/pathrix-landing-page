...
### Navigation
Fixed, full-width, transparent over the hero; inverts to a translucent ink pill (`rgba(16,30,42,.9)` + 10px blur) once scrolled past ~72% of the first viewport. Both states exist in the markup simultaneously so the bar is correct on first paint and without JavaScript — CSS attribute selectors (`[data-past="true"]`) drive the swap. Links are uppercase mono-adjacent sans (12px, 600 weight, 0.16em tracking) with a 1px `tugu-gold-lit` underline marking the active section. Below 900px, links collapse into a 44px pill burger button opening a full-screen ink menu with display-scale link type.
