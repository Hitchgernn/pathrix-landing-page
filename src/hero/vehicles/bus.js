...
export function createBus(paths) {
  ...
  group.name = "bus";
  group.scale.setScalar(0.85); // The ring road is narrow; the whole fleet has to fit one lane.
  ...
  return {
    ...
    update(delta) {
      ...
      // Centre of the lane. Offsetting it (the port had -1.5) put the bus flank
      // over the terraced tiles beside the plaza, which stand above the asphalt.
