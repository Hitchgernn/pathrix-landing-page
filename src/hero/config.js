...
/**
 * Every road vehicle shares this one speed, and therefore one lane.
 *
 * The asphalt band is 4.0 units wide but only ~2.65 of that is clear of terrain:
 * tiles are classified by their centre distance, so a tile centred just outside
 * the band still pokes into it and stands above the road. Two passing lanes of
 * these vehicles need ~2.60 before any gap at all, which does not fit — so the
 * road vehicles must never overtake. One shared speed keeps their arc gaps fixed
 * for good, and the four of them sit roughly a quarter of a lap apart.
 */
export const ROAD_SPEED = 2.3;
